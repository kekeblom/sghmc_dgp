...
class AdditivePatchKernel(object):
    ...
    """
    def __init__(self, base_kernel, patch_extractor, patch_weights=None):
        self.base_kernel = base_kernel
        self.patch_extractor = patch_extractor
        self.patch_length = patch_extractor.patch_length
        self.patch_count = patch_extractor.patch_count
        self.image_size = self.patch_extractor.input_size
        if patch_weights is None or patch_weights.size != self.patch_count:
            patch_weights = np.ones(self.patch_count, dtype=np.float64)
        self.patch_weights = tf.Variable(patch_weights, float_type, name='patch_weights')
    ...
    def K(self, ND_X, X2=None):
        ...
        if X2 is None:
            ...
            PNL_patches2 = self.patch_extractor.patches_PNL(self._reshape_X(X2))
...
class ConvKernel(AdditivePatchKernel):
    ...
    def K(self, ND_X, X2=None):
        ...
        # N * P x L
        patches = tf.reshape(self.patch_extractor.patches(NHWC_X), [-1, patch_length])

        if X2 is None:
            patches2 = patches
        else:
            patches2 = tf.reshape(self.patch_extractor.patches(X2), [-1, patch_length])
    ...
    def Kdiag(self, ND_X):
        ...
        # patches: N x patch_count x patch_length
        patches = self.patch_extractor.patches(NHWC_X)
        w = self.patch_weights
        W = w[None, :] * w[:, None]
        def sumK(p):
